...
OUTPUT_DIR = PROCESSED_DIR / "dl_tree_guided_moe_results"
...
def save_paper_table(results_df: pd.DataFrame):
    ...
    results_df[paper_cols].to_csv(OUTPUT_DIR / "paper_ready_dl_cost_aware_table.csv", index=False)
...
def main():
    ...
    for experiment in RUN_EXPERIMENTS:
        ...
        try:
            ...
            running_df.to_csv(OUTPUT_DIR / "dl_tree_guided_results_running.csv", index=False)
            with open(OUTPUT_DIR / "dl_tree_guided_results_running.json", "w", encoding="utf-8") as f:
    ...
    final_csv = OUTPUT_DIR / "dl_tree_guided_results_final.csv"
    final_json = OUTPUT_DIR / "dl_tree_guided_results_final.json"
    ...
    print(final_csv)
    print(final_json)
    print(OUTPUT_DIR / "paper_ready_dl_cost_aware_table.csv")
    print(results_df)
