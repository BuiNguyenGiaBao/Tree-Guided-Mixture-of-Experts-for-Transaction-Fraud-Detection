...
SELECTION_OBJECTIVE = "MCC"

OUTPUT_DIR = PROCESSED_DIR / "proposed_tree_guided_moe_tuning" / "B_kd030_sup010_mcc_seed42"
...
def run_experiment(experiment, train_loader, val_loader, test_loader, dims, cat_cardinalities, test_ds, review_meta_test, device):
        ...
        if selection_score > best_val_score:
            ...
            torch.save({
                ...
                "config": {
                    ...
                    "TUNING_CONFIG_NAME": "B_kd030_sup010_mcc_seed42",
                },
            }, ckpt_path)
        else:
            patience_counter += 1
    ...
    result = {
        ...
        "Tuning_Config": "B_kd030_sup010_mcc_seed42",
        "Selection_Objective": SELECTION_OBJECTIVE,
...
def save_paper_table(results_df: pd.DataFrame):
    ...
    results_df[paper_cols].to_csv(OUTPUT_DIR / "paper_ready_B_kd030_sup010_mcc_seed42.csv", index=False)
...
def main():
    ...
    print("Tuning config: B_kd030_sup010_mcc_seed42")
    ...
    for experiment in RUN_EXPERIMENTS:
        ...
        try:
            ...
            running_df.to_csv(OUTPUT_DIR / "B_kd030_sup010_mcc_seed42_running.csv", index=False)
            with open(OUTPUT_DIR / "B_kd030_sup010_mcc_seed42_running.json", "w", encoding="utf-8") as f:
    ...
    final_csv = OUTPUT_DIR / "B_kd030_sup010_mcc_seed42_final.csv"
    final_json = OUTPUT_DIR / "B_kd030_sup010_mcc_seed42_final.json"
    ...
    print(final_csv)
    print(final_json)
    print(OUTPUT_DIR / "paper_ready_B_kd030_sup010_mcc_seed42.csv")
    print(results_df)
