...
def replace_assignment(code: str, name: str, value: str) -> str:
    pattern = rf"^{name}\s*=\s*.*$"
    ...
    marker = re.search(r"^TARGET_COL\s*=.*$", code, flags=re.MULTILINE)
    if marker:
...
def patch_template_for_seed(template: str, cfg: dict) -> str:
    code = template

    # Ensure only the final proposed model is run.
    code = re.sub(
    ...
    code = replace_assignment(code, "RANDOM_STATE", str(cfg["seed"]))
    code = replace_assignment(code, "KD_WEIGHT", str(cfg["kd_weight"]))
    code = replace_assignment(code, "LAMBDA_SUPCON", str(cfg["lambda_supcon"]))

    # Keep the same training settings as your best config.
    code = replace_assignment(code, "LEARNING_RATE", "3e-4")
    code = replace_assignment(code, "DROPOUT", "0.30")
    code = replace_assignment(code, "EPOCHS", "20")
    code = replace_assignment(code, "PATIENCE", "5")

    # Separate output folder for each seed.
    code = re.sub(
        r'OUTPUT_DIR\s*=\s*PROCESSED_DIR\s*/\s*"[^"]+"',
        f'OUTPUT_DIR = PROCESSED_DIR / "proposed_tree_guided_moe_final_3seeds" / "{cfg["name"]}"',
        code,
        count=1,
    )

    # Add import path patch so generated scripts can import dependencies.
    import_patch = (
        "import sys\n"
    ...
    # Add seed/config info to final result if the result dict has Best_Val_PR_AUC.
    if '"Best_Val_PR_AUC": float(best_val_pr_auc),' in code and '"Config_Name":' not in code:
        code = code.replace(
            '"Best_Val_PR_AUC": float(best_val_pr_auc),',
            f'"Config_Name": "{cfg["name"]}",\n        "Config_Seed": {cfg["seed"]},\n        "Config_KD_Weight": {cfg["kd_weight"]},\n        "Config_Lambda_SupCon": {cfg["lambda_supcon"]},\n        "Best_Val_PR_AUC": float(best_val_pr_auc),',
            1,
        )
...
def copy_dependencies(scripts_dir: Path, generated_dir: Path) -> None:
    for filename in DEPENDENCY_FILES:
        ...
        if src.exists():
            ...
            print(f"[WARN] Missing dependency: {src}")
...
def find_result_csv(processed_dir: Path, cfg_name: str) -> Path | None:
    candidates = [
        processed_dir / "proposed_tree_guided_moe_tuning" / cfg_name / f"{cfg_name}_final.csv",
        processed_dir / "proposed_tree_guided_moe_tuning" / cfg_name / "proposed_only_results_final.csv",
        processed_dir / "proposed_tree_guided_moe_final_3seeds" / cfg_name / "proposed_only_results_final.csv",
        processed_dir / "proposed_tree_guided_moe_final_3seeds" / cfg_name / f"{cfg_name}_final.csv",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def combine_3seed_results(processed_dir: Path) -> None:
    summary_dir = processed_dir / "proposed_tree_guided_moe_final_3seeds" / "_summary"
    summary_dir.mkdir(parents=True, exist_ok=True)

    all_cfg_names = [
        "E_kd020_sup005_pr_auc_seed42",
        "E_kd020_sup005_pr_auc_seed2024",
        "E_kd020_sup005_pr_auc_seed3407",
    ]

    rows = []
    for cfg_name in all_cfg_names:
        p = find_result_csv(processed_dir, cfg_name)
        if p is None:
            print(f"[MISS] Result not found for {cfg_name}")
            continue

        df = pd.read_csv(p)
        df["Result_File"] = str(p)
        if "Config_Name" not in df.columns:
            df["Config_Name"] = cfg_name

        if "Config_Seed" not in df.columns:
            if cfg_name.endswith("seed42"):
                df["Config_Seed"] = 42
            elif cfg_name.endswith("seed2024"):
                df["Config_Seed"] = 2024
            elif cfg_name.endswith("seed3407"):
                df["Config_Seed"] = 3407

        rows.append(df)

    if not rows:
        print("[WARN] No seed results found to combine.")
        return

    combined = pd.concat(rows, ignore_index=True, sort=False)

    combined_path = summary_dir / "final_proposed_3seed_results.csv"
    ...
    for m in metric_cols:
        vals = pd.to_numeric(combined[m], errors="coerce").dropna()
        if len(vals) == 0:
            continue
    ...
    mean_std_path = summary_dir / "final_proposed_3seed_mean_std.csv"
    mean_std.to_csv(mean_std_path, index=False)
    print("[SAVED]", mean_std_path)
    ...
    paper_path = summary_dir / "paper_ready_final_proposed_3seed_table.csv"
...
def main():
    ...
    generated_dir = scripts_dir / "generated_final_extra_seed_runs"
    generated_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 100)
    print("FINAL PROPOSED MODEL - EXTRA SEEDS 2024 AND 3407")
    print("=" * 100)
    print("Scripts dir:", scripts_dir)
    print("Generated dir:", generated_dir)
    print("Processed dir:", PROCESSED_DIR)
    print("Config: KD_WEIGHT=0.20, LAMBDA_SUPCON=0.005")
    print("Seeds to run: 2024, 3407")
    print("Seed 42 is assumed to be already completed.")

    copy_dependencies(scripts_dir, generated_dir)

    status = []
    for cfg in EXTRA_SEED_CONFIGS:
        generated_script = generated_dir / f"{cfg['name']}.py"
        code = patch_template_for_seed(EMBEDDED_TEMPLATE, cfg)
        generated_script.write_text(code, encoding="utf-8")
        compile(code, str(generated_script), "exec")

        return_code = run_one(generated_script)
        status.append({"config": cfg, "return_code": return_code, "time": now()})

        status_path = PROCESSED_DIR / "proposed_tree_guided_moe_final_3seeds" / "_extra_seed_status.json"
        status_path.parent.mkdir(parents=True, exist_ok=True)
        status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2), encoding="utf-8")

        if return_code != 0:
            print(f"[FAILED] {cfg['name']} returned code {return_code}. Stop.")
            break

    combine_3seed_results(PROCESSED_DIR)
